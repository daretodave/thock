...
Travel is 4.0mm full stroke, 2.0mm actuation. That's the standard MX-format spec, which puts the Lanes in the same travel class as the <PartReference id="durock-t1" />, longer than the Drop Holy Panda X's 3.4mm, and longer still than the <PartReference id="boba-u4t" />'s shortened 3.2mm long-pole stroke. The 5-pin PCB mount delivers the reduced wobble that became an expectation across the mid-tier since 2022.
