...
The Lanes ships in a nylon bottom housing with a polycarbonate top, the same resin split that Gateron uses on the <PartReference id="gateron-oil-king" />. The top housing provides clarity and a slight hardness to the upstroke; the nylon bottom handles the bottom-out, converting what would be a sharp impact into a lower, more contained register. The POM stem is a different geometry from the Oil King's linear stem — the tactile step on the leading legs is the load-bearing difference — but the material choice is consistent across Gateron's current flagship lineup.
