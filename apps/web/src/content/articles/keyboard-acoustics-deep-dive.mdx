...
Spring ping is the acoustic sibling problem: a metallic ringing that appears on keystroke release and is often mistakenly attributed to the case. Spring ping originates inside the switch housing when the spring vibrates against the housing walls during decompression. The fix is thin oil (<Mono>Krytox 105</Mono>) on the spring coils, not case modification. The diagnostic: if a board rings distinctly on the upstroke but sounds controlled on the downstroke, the source is almost certainly spring ping, not the case.
