...
The spec on the HPX reads 60g actuation, 67g bottom-out, factory-fitted. A 7-gram delta between actuation and bottom-out is light by current-generation standards — the <a href="/article/gateron-oil-king-deep-dive">Oil King</a> runs roughly 55g into 65g across its travel, and the heavier modern tactiles (Sapphire, MMD Lavender) typically push their delta past 15 grams. What a small delta delivers is a flatter back-half of the curve: once the bump is crossed, the spring is not asking the finger for meaningfully more force to reach the bottom of the stroke. The bottom-out lands close in weight to where the tactile event resolved, and the upstroke returns with the same modest delta in reverse.
