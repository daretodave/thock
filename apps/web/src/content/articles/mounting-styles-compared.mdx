...
tags: [modding, acoustic, aluminum, polycarbonate, beginner, tray-mount]
...
<PartReference id="mode-sonnet" /> is a canonical modern example: a 75% aluminum case with the plate gasket-mounted top and bottom, shipping with hotswap PCBs and a brass weight option. The typing feel is the standard reference point for what gasket means in the current market — soft, uniform, low-pitched. Most enthusiast boards announced for group buy since 2023 are gasket-mounted, and the format has effectively become the default for new custom designs. The <PartReference id="kbd75v3" /> is a 75% counterpart in the same tier — gasket-mounted, hotswap PCB, QMK/VIA — and a common first recommendation for builders who want a function row without moving to a full TKL. The <PartReference id="class80" /> from MM Studio takes the same approach in a TKL chassis — CNC aluminum with both top-mount and gasket-mount configurations available from a single board — and is one of the more studied examples of how a multi-mode mount system changes the shape of the builder's decision.
...
A distinctive gasket variant worth naming on its own is the **burger mount** — a name that comes from the original <PartReference id="bakeneko" /> design, where the case clamshells around a single o-ring that runs the perimeter of the plate-PCB assembly instead of foam strips top and bottom. The o-ring still isolates the assembly from the case shell and still compresses under a keypress, just less than a well-built foam gasket does — the feel and sound land closer to gasket than to integrated, tighter and more uniform than a foam-strip build but with real give, not zero. Bakeneko-style boards remain the standard reference for what a well-executed budget gasket implementation sounds like, at a price point full custom gasket boards cannot reach.
...
Modern budget boards in the KBDFans QK series and similar unibody designs lean on this approach to deliver a tight, consistent sound at a price point top-mount customs cannot reach — no separate plate to manufacture or tune, just a milled case doing double duty.
...
If a builder wants the softest, deepest, most uniform sound and feel — and they are buying in the modern enthusiast custom market — they want gasket. The mode-sonnet and most of its contemporaries cover that space well, and the catalog is large enough that the only real question is which case material and which layout, not which mount. Builders who want that same gasket character at a fraction of the price should look at burger-mount boards like the Bakeneko instead — the earlier section covers the trade-off.
...
If a builder is shopping at the prebuilt end of the market and willing to mod, tray-mount is what they are getting whether the listing says so or not. The [PE foam mod](/article/pe-foam-mod) and [tape mod](/article/tape-mod) stack can take a tray-mount board most of the way to a satisfying sound; the [o-ring mod](/article/o-ring-mod) handles the standoff contact points that foam cannot reach; and [properly serviced stabilizers](/article/stabilizer-servicing-guide) address the rattle that neither foam nor o-ring touches. The floor is lower than gasket and the ceiling is lower too.

If a builder wants the tightest possible sound at the lowest possible price — and is willing to accept zero give in the typing surface — integrated plate is the answer. The QK-series delivers a sound character at $200 that most top-mount customs struggle to match at twice the price, with the trade-off paid entirely in firmness of feel.

The honest closing note is that mount style interacts with everything else the builder has chosen. A linear switch on a gasket board with PE foam and a thick MT3 set produces one kind of sound; the same switch on a top-mount board with bare PCB and a Cherry-profile ABS set produces a different one. The mount is a layer, not a verdict. For the plate variable that pairs with each of these mount styles — how aluminium, brass, FR4, POM, and polycarbonate each interact with tray, gasket, and top-mount geometries — see [plate materials, explained](/article/plate-materials-explained). The thock house take: pick the mount that matches the typing feel wanted, not the sound — the sound can be tuned around the mount, but the feel under the finger cannot.
