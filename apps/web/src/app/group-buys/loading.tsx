import { Container, Stack } from '@thock/ui'
import { PageSectionKicker } from '@/components/ui/PageSectionKicker'

export default function GroupBuysLoading() {
  return (
    <main id="main" className="flex-1">
      <Container as="header" className="py-12 sm:py-16">
        <Stack gap={4}>
          <PageSectionKicker>loading · group buys</PageSectionKicker>
          <div className="h-12 w-1/3 animate-pulse bg-surface" />
          <div className="h-6 w-2/3 max-w-[60ch] animate-pulse bg-surface" />
        </Stack>
      </Container>
      <Container as="section" className="pb-12 sm:pb-16">
        <Stack gap={5}>
          <div className="h-6 w-1/4 animate-pulse bg-surface" />
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="grid grid-cols-1 gap-4 border-t border-border py-5 sm:grid-cols-[140px_1fr_auto] sm:gap-6"
            >
              <div className="aspect-[4/3] animate-pulse bg-surface" />
              <div className="flex flex-col gap-3">
                <div className="h-4 w-1/4 animate-pulse bg-surface" />
                <div className="h-6 w-3/4 animate-pulse bg-surface" />
                <div className="h-4 w-2/3 animate-pulse bg-surface" />
              </div>
              <div className="h-6 w-20 animate-pulse bg-surface" />
            </div>
          ))}
        </Stack>
      </Container>
    </main>
  )
}
