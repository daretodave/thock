...
/**
 * One resolved part from `frontmatter.mentionedParts` — same shape
 * as `@thock/content`'s `ResolvedPart`. Re-declared here because we
 * cannot pull the runtime helper from the package (it walks
 * filesystem via @thock/data), only its types.
 */
...
/**
 * Resolves an article's `mentionedParts` against the manifest data.
 * Same semantics as `@thock/content`'s `getReferencedParts`: drops
 * references whose data slug doesn't resolve, preserves frontmatter
 * order.
 */
export function getReferencedParts(article: Article): ResolvedPart[] {
  const out: ResolvedPart[] = []
  for (const ref of article.frontmatter
    .mentionedParts as ArticlePartReference[]) {
    if (ref.kind === 'switch') {
      const record = getSwitchBySlug(ref.slug)
      if (record)
        out.push({ id: ref.id, kind: 'switch', slug: ref.slug, record })
    } else if (ref.kind === 'keycap-set') {
      const record = getKeycapSetBySlug(ref.slug)
      if (record)
        out.push({ id: ref.id, kind: 'keycap-set', slug: ref.slug, record })
    } else if (ref.kind === 'board') {
      const record = getBoardBySlug(ref.slug)
      if (record)
        out.push({ id: ref.id, kind: 'board', slug: ref.slug, record })
    }
  }
  return out
}
