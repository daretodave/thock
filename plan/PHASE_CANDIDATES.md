...
> Last pass: 2026-05-14 at commit 047d2f2
...
- proposed: 2026-05-14, expand pass 9
- source signals:
...
### [ ] [score 5.5] A11y Phase B systematic completion — sweep all remaining text-text-3 at small text sizes site-wide
- proposed: 2026-05-14, expand pass 10
- source signals:
  - **Commit pattern**: 8 consecutive commits in the pass-10 window all performing identical `text-text-3` → `text-text-2` substitution at 12-14px contexts (byline `d486ad5`, footer `9998ae5`, attribution `5753b7a`, TrendingTile `dc99bef`, rail headings `9f857d5`, GroupBuysWidget `9cc6048`, TrackerTable/ArchiveStrip `72119d8`, figcaptions `531937e`). Same root cause, same fix, same WCAG rule (1.4.3 Contrast Minimum AA).
  - **Remaining scope**: `grep -rn "text-text-3"` across `apps/web/src/{components,app}/ + packages/content/src/mdx/` yields 40+ remaining `text-micro|text-small` instances: GroupBuyRow (kind/status/metadata), PillarHero article-count, PartSpec/PartHero spec keys and metadata, MentionedInArticles, SuggestedArticles, GroupBuyCountdownRow, NewsletterArchive, CitationIndex, SourceCounts, ArticleCard fallback, PageStub, ArticleResult (search), Caption/PullQuote/Callout MDX components, plus multiple route pages (group-buys, sources, tags, news, guides, trends, tracker, parts, not-found).
  - **Expand signal A**: 3+ findings under same a11y category (WCAG 1.4.3, text-text-3 at small text) → suggests a coordinated pass rather than 7-10 more iterate ticks.
- rationale: The iterate loop is draining these correctly but slowly (one component per tick). The fix pattern is proven and mechanical — `text-text-3` → `text-text-2` at any `text-micro`/`text-small` context on the standard dark background. A systematic sweep of all 40+ remaining instances in one commit (with contextual review to exclude any that legitimately sit on lighter surface colors) would close WCAG 1.4.3 compliance site-wide, free ~7-10 future iterate ticks for higher-value work, and add batch regression guards per affected surface rather than one-per-component. MDX components (`Caption`, `PullQuote`, `Callout`) require extra care — some text-text-3 uses there may be intentional editorial muting against the article's lighter prose-block background.
- proposed scope (1 phase):
  1. Audit: grep all `text-text-3 + (text-micro|text-small)` combos across `apps/web/src/` + `packages/content/src/mdx/`. Annotate each with its background context (standard dark bg vs prose-block surface vs surface color).
  2. Fix: swap `text-text-3` → `text-text-2` at all confirmed WCAG-failing contexts. Leave MDX `Caption`/`PullQuote` as Considered edge cases if they sit on a genuinely lighter prose block background — document the decision.
  3. Regression guards: add `data-testid` + `AxeBuilder.include()` scoped assertion per *surface* (not per instance) — group by page/component family, not one guard per line. 10-15 new guards, not 40.
