...
> Last pass: 2026-05-17 at commit 9b6a963
...
- [score 4.5] Tracker null-articleSlug "article pending" visual treatment — W21 snapshot has 15 rows with real direction/score data but no `articleSlug`. Phase 8 "early data muted styling" was designed for absent data, not present-data-no-article. A rendering variant distinguishing "data present, article pending" from "no data yet" would be honest UX, especially after Phase 31's Monday gate makes null-slug rows routine (12–18 per snapshot). Iterate-shaped (component-level change to `TrackerRow`); re-evaluate if Rule 2 rows accumulate at HIGH signal strength.
