...
- [x] Phase 15 — Newsletter signup + RSS finalize (Buttondown embed at handle `thock`, `/newsletter` with empty-state archive, GTM `GTM-58T839ZD` wired via Next `<Script strategy="afterInteractive">`, handwritten RSS 2.0 validator + 6 e2e feeds, Footer retrofit drops the placeholder aria-label) — pending commit
