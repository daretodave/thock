...
- [ ] Phase 47 — `/quiz/keycap-set` keycap-set recommender (score 5.5, pass-125 candidate; keycap-sets crossed 10 records). Mirrors phase-33 `/quiz/switch`: `apps/web/src/app/quiz/keycap-set/page.tsx` (+ `loading.tsx` with `<main id="main">`), 4–5 weighted questions over `getAllKeycapSets()` via a pure `recommendKeycapSet(answers, catalog)` helper with unit tests, ResultCard links to `/part/keycap-set/[slug]`, `WebApplication` JSON-LD, sitemap + `canonical-urls` + `page-reads` + `apps/e2e/tests/quiz.spec.ts` extended, `/quiz/keycap-set` in `bearings.md`. No schema change. Brief drafted on-demand.
