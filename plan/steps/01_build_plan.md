...
**Promoted candidate (phase 50, promoted via /oversight 2026-08-23 from PHASE_CANDIDATES expand pass 207/319 — the [7.5] trend-snapshot data-quality gate, 25 confirmed instances):**
- [x] Phase 50 — Trend-snapshot data-quality gate (score 7.5, pass-207 candidate, 25 instances confirmed through pass 319). Ships `scripts/trend-snapshot-quality-check.mjs` (dry-run / `--write` / `--json`, sibling-survey shape) with four checks over `data/trends/*.json`: (1) row `note`/vendor-facing text cross-checked against `data/vendors` + `data/group-buys` names (name-casing drift, excluding "Drop" — common-word collision); (2) where `articleSlug` is set, the linked article's full content (frontmatter + body) must reference the row's topic (keyword heuristic); (3) notes stating a linked group buy's status must agree with the `data/group-buys/*.json` record's `status` field (name-gated, sentence-segment scoped — no date arithmetic, which false-positived 14/14 in a corpus dry run); (4) `direction` must agree with the sign of the `spark` array's last delta (`flat` tolerates |delta| ≤ 2), and week-over-week spark arrays must be shift-and-append continuous. `skills/march.md` Step 3.5 amendment runs the check immediately after the Monday snapshot write and **hard-fails the snapshot commit on errors** (gate, not survey). 15 unit tests under `scripts/__tests__/`. One-time corpus scan filed 3 residual AUDIT rows (all direction/spark sign contradictions). No URL/schema change. **Brief: `phase_50_trend_snapshot_quality_gate.md`.** — `fbae462a`
