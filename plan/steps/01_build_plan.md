...
- [ ] Phase 50 — Trend-snapshot data-quality gate (score 7.5, pass-207 candidate, 25 instances confirmed through pass 319). Ships `scripts/trend-snapshot-quality-check.mjs` (dry-run / `--write` / `--json`, sibling-survey shape) with four checks over `data/trends/*.json`: (1) row `note`/vendor-facing text cross-checked against `data/vendors` + `data/group-buys` names (misattribution, spelling drift like UniKeys/Unikeys); (2) where `articleSlug` is set, the linked article's title/tags must reference the row's topic (keyword heuristic); (3) notes stating a group-buy count/date/status must agree with the linked `data/group-buys/*.json` record; (4) `direction` must agree with the sign of the `spark` array's last delta, and week-over-week spark arrays must be shift-and-append continuous (no fabricated discontinuities). `skills/march.md` Step 0.5 amendment runs the check immediately after the Monday snapshot write and **hard-fails the snapshot commit on errors** (gate, not survey). Unit tests: one positive + one control per check under `scripts/__tests__/`. One-time corpus scan in the ship commit files residual AUDIT rows. No URL/schema change. Brief drafted on-demand by `/ship-a-phase`.
