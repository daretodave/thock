...
- [ ] Phase 26 — Accessibility audit pass — Phase A discovery walker (score 6.5, pass-1 candidate). Ships a script at `apps/web/scripts/a11y-audit.mts` (or e2e equivalent at `apps/e2e/tests/a11y.spec.ts`) that walks every canonical URL at desktop + mobile breakpoints and files `[a11y]` rows into `plan/AUDIT.md`. Path lock (axe-core vs hand-checklist) deferred to `/plan-a-phase phase 26` time — recommend playwright-axe given playwright is already wired into `pnpm verify`. Discovery output drains in subsequent `/iterate` ticks (Phase B from the original 2-phase mini-plan — not a separate phase row, the drain *is* the iterate loop). **Brief drafted on-demand by `/ship-a-phase`.**
- [ ] Phase 27 — Trends Tracker multi-week archive surface — Phase A only (score 6.0, pass-4 candidate). New route `apps/web/src/app/trends/tracker/[week]/page.tsx` rendering any `data/trends/<YYYY-WNN>.json`; `getAllTrendSnapshots()` loader; existing `/trends/tracker` "latest" view gains `← older weeks` affordance + sparkline strip showing last 4–8 weeks of category-direction signal. Sitemap entries; CollectionPage JSON-LD per week. e2e in `apps/e2e/tests/tracker-archive.spec.ts`. **Phase B (recurring snapshot cadence) remains in PHASE_CANDIDATES Considered until Phase A ships — path lock between cloud `/schedule` cron vs `skills/march.md` amendment deferred to that point.** Brief drafted on-demand by `/ship-a-phase`.
- [ ] Phase 28 — `/tags` index page (score 5.5, pass-1 candidate). New route `apps/web/src/app/tags/page.tsx` rendering every tag from `articleAllTags(articles)` grouped by the five categories (`vendor`, `part`, `layout`, `profile`, `community` per `design/decisions.jsx`). Categorical-tinted heading per group; each chip links to `/tag/<slug>`. CollectionPage + ItemList JSON-LD; sitemap entry. Update `/tag/[slug]` back-link from `← all tags` → `/tags` (relabel becomes truthful — drains CRITIQUE pass-2 row e270ced). New e2e in `apps/e2e/tests/tags.spec.ts`. **Brief drafted on-demand by `/ship-a-phase`.**
- [ ] Phase 29 — Group-buy archive `/group-buys/past` (score 5.5, pass-6 candidate). New loader `getAllClosedGroupBuys()` returning records whose `endDate` is in the past, sorted desc. New route `apps/web/src/app/group-buys/past/page.tsx` rendering archive list with vendor, buy name, status pill (`CLOSED` / `SHIPPED`), close date, and a link to the companion article if `relatedArticle` is set. Small "view past buys" affordance on `/group-buys`. Sitemap entry + CollectionPage JSON-LD. e2e in `apps/e2e/tests/group-buys.spec.ts`. **Brief drafted on-demand by `/ship-a-phase`.**
