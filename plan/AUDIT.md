...
### [ ] [engineering] [4.9] ship-content dispatch stuck in a duplicate-issue loop for the deep-dives content-gap row — 9 GitHub issues opened, 0 articles shipped, ~10.5h and counting
- category: engineering
...
> Filed by the nightly `/digest` pulse review, not a breadth-check (`pnpm verify`) failure — the full verify gate this tick is green (see Breadth verdict in `plan/DIGEST.md`). This is a distinct failure class from the standing `[6.3]` blocked-cloud-permission row: that one is a crashed/failed action the crash-issue gate should have caught and didn't; this one is a *clean* SDK-level stop on every attempt, so no crash-detection mechanism was ever going to catch it. Proposing a `plan/PHASE_CANDIDATES.md` candidate for the Step-3 persistence fix (the well-scoped half); the harder diagnostic half is noted as follow-up, not blocking the candidate.
