...
/**
 * Regression guard for user-jot 11d932d (CRITIQUE.md
 * "/article/* — cheat-sheet / Callout block has no margin-bottom;
 * next H2 collides"). The fix paired the SerifH2 top margin with
 * a Callout block-margin so the gap survives CSS margin-collapse
...
describe('MDX prose spacing — Callout + h2', () => {
  ...
  it('SerifH2 renders with mt-16 (64px) so it never reads as flush', () => {
    const H2 = mdxComponents.h2
    const { container } = render(<H2>The three families</H2>)
    const h2 = container.querySelector('h2')
    expect(h2).not.toBeNull()
    expect(h2!.className).toContain('mt-16')
